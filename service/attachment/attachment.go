...
func NewAttachmentService(basic *service.BasicService, dao DAO, storage Storage) *Service {
	return &Service{
		BasicService: basic,
		dao:          dao,
		storage:      storage,
...
func (a *Service) SaveFiles(ctx context.Context, files []*multipart.FileHeader, uid uint) ([]*response.UploadAttachmentResponse, error) {
	//var info = make([]*AttachmentInfo, 0, len(files))
	//projectDir, temp := os.Getwd()
	//if temp != nil {
	//	return nil, temp
	//}
	//targetDir := a.conf.System.UploadAttachmentDir
...
func (a *Service) SaveFile(ctx context.Context, file *multipart.FileHeader, uid uint) (*response.UploadAttachmentResponse, error) {
